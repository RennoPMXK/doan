...
function displayUserDict() {
    ...
    document.getElementById('userlist').style.height = UserDict.length*2.7 + 'rem';
...
function displayProductDict() {
    ...
    document.getElementById('productlist').style.height = productDict.length*3.6 + 'rem';
...
function editImg1Step1(position) {
    ...
    document.getElementById('modify-img').innerHTML = `
        ...
        <div id='display_image'></div>
        <button type='submit' id='saveImageButton' onclick='saveImageAndTurnBack();'>Save</button>
        <button type='submit' id='cancelButton' onclick='cancelAndTurnBack1(${position})'>Cancel</button>
        <button type='submit' id='discardButton' onclick='discardAnh(${position}) '>Discard</button>
    `;
    const image_input = document.querySelector('#image_input');
    var uploaded_image = '';
    previousDir1 = productDict[position].anh1;
    image_input.addEventListener('change', function () {
        const reader = new FileReader();
        reader.addEventListener('load', () => {
            var productDict = JSON.parse(localStorage.getItem('product'));
...
function editImg2Step1(position) {
    ...
    document.getElementById('modify-img').innerHTML = `
        ...
        <div id='display_image'></div>
        <button type='submit' id='saveImageButton' onclick='saveImageAndTurnBack();'>Save</button>
        <button type='submit' id='cancelButton' onclick='cancelAndTurnBack2(${position})'>Cancel</button>
        <button type='submit' id='discardButton' onclick='discardAnh(${position}) '>Discard</button>
    `;
    const image_input = document.querySelector('#image_input');
    var uploaded_image = '';
    previousDir2 = productDict[position].anh2;
    image_input.addEventListener('change', function () {
        const reader = new FileReader();
        reader.addEventListener('load', () => {
            var productDict = JSON.parse(localStorage.getItem('product'));
...
function editImg3Step1(position) {
    ...
    document.getElementById('modify-img').innerHTML = `
        ...
        <div id='display_image'></div>
        <button type='submit' id='saveImageButton' onclick='saveImageAndTurnBack();'>Save</button>
        <button type='submit' id='cancelButton' onclick='cancelAndTurnBack3(${position})'>Cancel</button>
        <button type='submit' id='discardButton' onclick='discardAnh(${position}) '>Discard</button>
    `;
    const image_input = document.querySelector('#image_input');
    var uploaded_image = '';
    previousDir3 = productDict[position].anh3;
    image_input.addEventListener('change', function () {
        const reader = new FileReader();
        reader.addEventListener('load', () => {
            var productDict = JSON.parse(localStorage.getItem('product'));
...
//         <button type='submit' id='saveImageButton' onclick='saveImageAndTurnBack();'>Save</button>
//         <button type='submit' id='cancelButton' onclick='cancelAndTurnBack1(${position});'>Cancel</button>
//         <button type='submit' id='discardButton' onclick='discardAnh1(${position}) '>Discard</button>
//     `;
//     const image_input = document.querySelector('#image_input');
//     var uploaded_image = '';
//     previousDir1 = productDict[position].anh1;
...
//         <button type='submit' id='saveImageButton' onclick='saveImageAndTurnBack();'>Save</button>
//         <button type='submit' id='cancelButton' onclick='cancelAndTurnBack2(${position});'>Cancel</button>
//         <button type='submit' id='discardButton' onclick='discardAnh2(${position}) '>Discard</button>
//     `;
//     const image_input = document.querySelector('#image_input');
//     var uploaded_image = '';
//     previousDir2 = productDict[position].anh2;
...
//         <button type='submit' id='saveImageButton' onclick='saveImageAndTurnBack();'>Save</button>
//         <button type='submit' id='cancelButton' onclick='cancelAndTurnBack3(${position});'>Cancel</button>
//         <button type='submit' id='discardButton' onclick='discardAnh3(${position}) '>Discard</button>
//     `;
//     const image_input = document.querySelector('#image_input');
//     var uploaded_image = '';
//     previousDir2 = productDict[position].anh2;
...
function cancelAndTurnBack(position) {
    var productDict = JSON.parse(localStorage.getItem('product'));
    if (previousDir != '') productDict[position].anh = previousDir;
    previousDir = '' ;
    localStorage.setItem('product', JSON.stringify(productDict));
    showOrHideOrFlex('modify-img', 'none');
    showOrHideOrFlex('add-nick-field-id', 'flex');
...
function cancelAndTurnBack1(position) {
    var productDict = JSON.parse(localStorage.getItem('product'));
    if (previousDir1 != '') productDict[position].anh1 = previousDir1;
    previousDir1 = '' ;
    localStorage.setItem('product', JSON.stringify(productDict));
    showOrHideOrFlex('modify-img', 'none');
    showOrHideOrFlex('add-nick-field-id', 'flex');
...
function cancelAndTurnBack2(position) {
    var productDict = JSON.parse(localStorage.getItem('product'));
    if (previousDir2 != '') productDict[position].anh2 = previousDir2;
    previousDir2 = '' ;
    localStorage.setItem('product', JSON.stringify(productDict));
    showOrHideOrFlex('modify-img', 'none');
    showOrHideOrFlex('add-nick-field-id', 'flex');
...
function cancelAndTurnBack3(position) {
    var productDict = JSON.parse(localStorage.getItem('product'));
    if (previousDir3 != '') productDict[position].anh3 = previousDir3;
    previousDir3 = '' ;
    localStorage.setItem('product', JSON.stringify(productDict));
    showOrHideOrFlex('modify-img', 'none');
    showOrHideOrFlex('add-nick-field-id', 'flex');
...
function showZone (id) {
    if (id == 'my-chart-1-container-id') {
        // document.getElementById('my-chart-1-container-id').style.zIndex = 100;
        // document.getElementById('my-chart-2-container-id').style.zIndex = 99;
        // document.getElementById('my-chart-3-container-id').style.zIndex = 98;
        // document.getElementById('my-chart-4-container-id').style.zIndex = 97;

        showOrHideOrFlex('my-chart-1-container-id', 'block');
        showOrHideOrFlex('my-chart-2-container-id', 'none');
        showOrHideOrFlex('my-chart-3-container-id', 'none');
        showOrHideOrFlex('my-chart-4-container-id', 'none');
    } else if (id == 'my-chart-2-container-id') {
        // document.getElementById('my-chart-1-container-id').style.zIndex = 99;
        // document.getElementById('my-chart-2-container-id').style.zIndex = 100;
        // document.getElementById('my-chart-3-container-id').style.zIndex = 98;
        // document.getElementById('my-chart-4-container-id').style.zIndex = 97;
        showOrHideOrFlex('my-chart-1-container-id', 'none');
        showOrHideOrFlex('my-chart-2-container-id', 'block');
        showOrHideOrFlex('my-chart-3-container-id', 'none');
        showOrHideOrFlex('my-chart-4-container-id', 'none');
    } else if (id == 'my-chart-3-container-id') {
        // document.getElementById('my-chart-1-container-id').style.zIndex = 98;
        // document.getElementById('my-chart-2-container-id').style.zIndex = 99;
        // document.getElementById('my-chart-3-container-id').style.zIndex = 100;
        // document.getElementById('my-chart-4-container-id').style.zIndex = 97;
        showOrHideOrFlex('my-chart-1-container-id', 'none');
        showOrHideOrFlex('my-chart-2-container-id', 'none');
        showOrHideOrFlex('my-chart-4-container-id', 'block');
        showOrHideOrFlex('my-chart-3-container-id', 'none');
    } else if (id == 'my-chart-4-container-id') {
        // document.getElementById('my-chart-1-container-id').style.zIndex = 97;
        // document.getElementById('my-chart-2-container-id').style.zIndex = 99;
        // document.getElementById('my-chart-3-container-id').style.zIndex = 98;
        // document.getElementById('my-chart-4-container-id').style.zIndex = 100;
        showOrHideOrFlex('my-chart-1-container-id', 'none');
        showOrHideOrFlex('my-chart-2-container-id', 'none');
        showOrHideOrFlex('my-chart-4-container-id', 'none');
        showOrHideOrFlex('my-chart-3-container-id', 'block');
    }
}

function chartForMen (idChart) {
    console.log('chart for men drew');
    var productlist = JSON.parse(localStorage.getItem('product'));
    var myLabel = [];
    var myData = [];
    for (let i = 0; i < productlist.length; i++) 
...
function chartForWomen (idChart) {
    console.log('chart for women drew');
    var productlist = JSON.parse(localStorage.getItem('product'));
    var myLabel = [];
    var myData = [];
    for (let i = 0; i < productlist.length; i++) 
