...
//NOTE:
// them kiem tra form add product
////// lam on bo sung cho redirect logout

// document.getElementById("user-button").addEventListener("click", displayUserPanel());
// document.getElementById("dash-button").addEventListener("click", displayDashboard);
...
// display products in content-view
...
function displayUserDict() {
    ...
    // document.getElementById('add-nick-but').style.display = "block";
    // document.getElementById("productlist").style.display = "none";
    // document.getElementById('add-product-but').style.display = "none";
	...
	for(var i = 0 ; i < UserDict.length; i++)
		cache += `<div class='table-control'>
            ...
            <button style="margin-left: 28rem" id="deletebut${i}" class="delete" onClick="deleteUser('${UserDict[i].username}')">❌</button>
            <button style="margin-left: 33rem" id="editbut" class="edit" onClick="editUser('${i}')"><i class="fas fa-edit"></i></button>
            <div style="margin-left: 36rem">${UserDict[i].datesignup}</div>
            <button class="logout-but" onclick="logOut('${UserDict[i].username}')"><i class="fas fa-sign-out-alt"></i></button>
        </div>`
...
function editUser(position) {
    ...
    document.getElementById('add-nick-but').addEventListener("click", () => {
        ...
        // check if username already in the array 
        ...
        if (password!='')
            userDict[position].password = pass ;
        if (fullname!='')
    ...
    // pass usernameToEdit to addNick()
}
...
function addNick() {
    if (editFlag == false) {
        ...
        var temp = {username, password, fullname, datesignup};
        userDict.push(temp);
        localStorage.setItem('user', JSON.stringify(userDict));
        displayUserDict();
    }
...
function displayProductDict() {
	...
	for (var i = 0 ; i < productDict.length; i++)
		cache += `<div class='table-control'>
            ...
            <button style="margin-left: 31.5rem" id="deletebut${i}" class="delete" onClick="deleteProduct('${productDict[i].id}')">⨉</button>
            <button style="margin-left: 36rem" id="editbut" class="edit" onClick="editProduct('${i}')">⩥</button>
            <button style="margin-left: 38rem" id="editImg-id" class="editImg" onClick="editImgStep1('${i}')"><i class="fas fa-plus"></i></button>
...
function saveImageAndTurnBack () {
    ...
    // document.getElementById('modify-img').style.display = 'none';
    // document.getElementById('add-nick-field-id').style.display = 'flex';
    // document.getElementById('productlist').style.display = 'block';
}
...
// Purpose: guide user to see input form interface
