...
function AddToDetailBill(mahd , makh){
     ...
     var cartItemList = document.getElementsByClassName('cart-table-list')[0]
               ...
               var x = Math.round(total)*1000000
...
function thanhtoan() {
     ...
     if (name.length == 0 || email.length == 0 || city.length == 0 || district.length == 0 || ward.length == 0 || address.length == 0) {
         if (name.length == 0) {
...
//     document.getElementById("nav").style.backgroundColor= '#F97E6C';

//   } else {
//     document.getElementById("nav").style.backgroundColor = "black";
//   var x =  document.getElementsByClassName('navbar-menu-item')
...
function LienHe() {
     ...
     if (name.length == 0 || email.length == 0 || phone.length < 10 || subject.length == 0 || !(email.includes('@'))) {
         if (name.length == 0) {
         ...
         if (subject.length != 0) {
          ...
          alert("Chúng tôi sẽ sớm liên lạc với bạn ! . Tự động quay về trang chủ")
          PageHome()
     }
 }