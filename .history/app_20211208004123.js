     ...
     var bill_arr = JSON.parse(localStorage.getItem('bill'))
      let mahd = Math.random().toString(36).replace(/[^a-z]+/g, '').substr(0, 8).toUpperCase()
          ...
          var Bill1 = {
               ...
               ngayhd : new Date(),
               tongtien : AddToDetailBill(mahd)
               };
     
               bill_arr.push(Bill1)
               
               AddToDetailBill(mahd , makh)
...
function InnerBill(makh){
     ...
     for(let i = 0; i < bill_arr.length; i++){
          ...
          for(let j = 0; j < detailBill_arr.length; j++){
                 ...
                 var detail = ''
                   if(bill_arr[i].makh === makh && bill_arr[i].mahd == detailBill_arr[j].mahd){
                        
                        detail.innerHTML+= `
                              ${detailBill_arr[j].masp} x ${detailBill_arr[j].soluong}
                        `
                        content.innerHTML = `
                        <th id="bill-num" >
                          ${bill_arr[i].mahd}
