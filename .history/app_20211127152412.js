...
function updateCart() {
     ...
     for(let i = 0; i < cartItemList.length; i++) {
          ...
          for(let i = 0; i < cartRows.length; i++) {
               ...
               console.log(cartRow)
               var quantityElm = cartRow.getElementsByClassName('quantityCart')
               for(var j = 0; j < quantityElm.length; j++){
                    var priceElm = cartRow.querySelector('#price')
                    var price = parseFloat(priceElm.innerText.replace('đ', ''))
                    var quantity = quantityElm[i].value
     ...
     document.querySelector(".cart-shopping-total h3").innerHTML = `${total}đ `
