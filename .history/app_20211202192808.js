 ...
 document.querySelector('.btn-modal-cart').addEventListener('click', function(){
     var x = document.getElementsByClassName('cart-row')
      if(x.length==0){
           ...
           location.href = './doan_thanhtoan.html'
      }
 })
        ...
        flag = 1   
           ...
           if(flag==1){
               var user1 = {
                    ...
                    password: password , 
                    email : email,
                    makh : Math.random().toString(36).replace(/[^a-z]+/g, '').substr(0, 8),
                    };
...
function AddToBill(){
     ...
     var fullname = document.querySelector('.account span').innerHTML
     var userArray = JSON.parse(localStorage.getItem('user'));
     for(var i=0; i<userArray.length; i++){
          if(userArray[i].username == fullname){
               console.log(userArray[i].mahd)
          }
     } 
