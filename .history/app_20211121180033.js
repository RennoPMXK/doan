...
function renderProduct(arr) {
   ...
   for(let i = 0; i < arr.length; i++){
        ...
        <img src= ${arr[i].anh} alt="" class="product-img">
               ...
               <i class='bx bx-shopping-bag product-icon'onclick="ModalCart('${arr[i].anh}', '${arr[i].ten}', '${arr[i].giamoi}')"></i>
...
function ModalCart(anh,ten,gia){
     ...
     document.querySelector('.cart-img img').src = anh 
     document.querySelector('.modal-cart-top span').innerText = ten 
     document.querySelector('.modal-cart-bottom h3 p').innerText = 1
     document.querySelector('.cart-content p').innerText = ten 
     document.getElementById('price').innerText = gia
     document.getElementById('price-total').innerText = gia
}

function updateCart() {
     var cartItemList = document.querySelector('.cart-table-list')
     var cartRows = cartItemList.querySelectorAll('.cart-row')
     var total = 0 ;
     for(let i = 0; i < cartRows.length; i++) {
          var cartRow = cartRows[i]
          var price1 = cartRow.getElementById('price')
          var quantityElm = cartRow.getElementById('quantity')
          console.log(quantityElm)
          var quantity = quantityElm.value
          total = total + (price1 * quantity)
     }

     document.querySelector(".sum-total h3").innerHTML = total
